...
export function setupToolbar(table) {
    ...
    btnCreate?.addEventListener("click", () =>
        alert("Crear movimiento (implementar modal)")
    );
     ...
     * 2. Carga el código del cliente en el input
     * 3. Entra en modo edición (muestra campos ocultos)
     * 4. Muestra el panel de detalles
     */
    btnEdit?.addEventListener("click", () => {
        if (!selectedRowData()) return alert("Seleccione un movimiento");
        
        // Pre-cargar el código del cliente en el input
        const clienteEl = document.getElementById("cliente");
        if (clienteEl)
            clienteEl.value = selectedRowData()?.tercero?.codcli ?? "";
        
        // Activar modo de edición (muestra campos ocultos)
        enterEditMode();
