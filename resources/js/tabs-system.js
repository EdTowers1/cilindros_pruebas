 ...
 * - API pública: Objeto TabSystem con métodos públicos
 *
 * Uso:
 * - Importar el módulo: import TabSystem from './tabs-system.js';
 * - Inicializar: TabSystem.init();
 * - Abrir módulo: TabSystem.abrirModulo('cilindros', 'Cilindros')
 * - Crear tab personalizado: TabSystem.addCustomTab('<p>Contenido</p>', 'Mi Tab', true)
 */
...
function loadTabContent(url, pane, btnTab) {
    if (state.tabCache[url]) {
        ...
        executeScripts(pane);
        if (pane.querySelector('#movimientos-table')) initMovimientos();
